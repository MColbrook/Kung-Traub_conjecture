...
theorem analyticAt_log_one_add {t : ℝ} (ht : -1 < t) :
  ...
  exact (analyticAt_const.add analyticAt_id).log (by dsimp; linarith)
...
theorem analyticOnNhd_log_one_add :
  ...
  intro t ht
  exact analyticAt_log_one_add ht

/-- The inverse is analytic at the root value zero. -/
theorem analyticAt_log_one_add_zero :
    AnalyticAt ℝ (fun t : ℝ => Real.log (1 + t)) 0 :=
  hasFPowerSeriesAt_log_one_add.analyticAt
...
theorem normalized_log_one_add_derivative (n : ℕ) :
    iteratedDeriv n (fun t : ℝ => Real.log (1 + t)) 0 / (n.factorial : ℝ) =
      -(-1 : ℝ) ^ n / (n : ℝ) := by
  have hseries := analyticAt_log_one_add_zero.hasFPowerSeriesAt.eq_formalMultilinearSeries
    hasFPowerSeriesAt_log_one_add
  have hcoeff := congrArg (fun p : FormalMultilinearSeries ℝ ℝ ℝ => p.coeff n) hseries
  simpa only [FormalMultilinearSeries.coeff_ofScalars] using hcoeff
...
theorem normalized_log_one_add_derivative_succ_succ (j : ℕ) :
  ...
  rw [normalized_log_one_add_derivative]
...
theorem log_one_add_hermite_coefficient_tendsto {X : Type*} {l : Filter X}
    ...
    Tendsto (fun u => confluentDividedDifference (fun t : ℝ => Real.log (1 + t))
      ...
      (𝓝 ((-1 : ℝ) ^ (j + 1) / ((j : ℝ) + 2))) := by
  have h := hermiteDividedDifference_tendsto j analyticAt_log_one_add_zero nodes hnodes
  simpa only [normalized_log_one_add_derivative_succ_succ] using h
...
theorem tendsto_exp_sub_one_div :
    Tendsto (fun x : ℝ => (Real.exp x - 1) / x)
      (𝓝[≠] (0 : ℝ)) (𝓝 (1 : ℝ)) := by
  simpa only [Real.exp_zero, zero_add, smul_eq_mul, div_eq_mul_inv, mul_comm] using
