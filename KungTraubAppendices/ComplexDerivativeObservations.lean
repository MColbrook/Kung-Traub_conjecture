...
theorem iteratedDeriv_complexFiniteAffineFamily {d : ℕ} {f : ℂ → ℂ}
  ...
  have hterms : ∀ i ∈ (Finset.univ : Finset (Fin d)),
      ContDiffAt ℂ k (fun t => u i * basis i t) z :=
    fun i _ => contDiffAt_const.mul (hbasis i)
  unfold complexFiniteAffineFamily
  rw [iteratedDeriv_fun_add hf (.sum hterms), iteratedDeriv_fun_sum hterms]
  simp only [iteratedDeriv_const_mul_field, complexDerivativeObservationLinearMap_apply]
...
theorem complexDerivativeAffineObservation_location {d : ℕ} (f : ℂ → ℂ)
    ...
    (complexDerivativeAffineObservation f basis q).location = q.location := by cases q <;> rfl
...
theorem complexDerivativeAffineObservation_idle_answer {d : ℕ} (f : ℂ → ℂ)
  ...
  simp [complexDerivativeAffineObservation, ComplexAffineObservation.answer]
...
theorem complexDerivativeAffineObservation_answer {d : ℕ} {f : ℂ → ℂ}
  ...
  cases q with
  | idle => simp [complexDerivativeAffineObservation_idle_answer, ComplexQuery.answer]
  | derivative z k =>
    have hk : (k : WithTop ℕ∞) ≤ ∞ := WithTop.coe_le_coe.mpr le_top
    exact (iteratedDeriv_complexFiniteAffineFamily u z k (hf.contDiffAt.of_le hk)
...
theorem complexDerivativeAffineObservation_answer_of_analytic {d : ℕ} {f : ℂ → ℂ}
    ...
    (u : EuclideanSpace ℂ (Fin d)) :
    (complexDerivativeAffineObservation f basis q).answer u = q.answer (complexFiniteAffineFamily f basis u) := by
  apply complexDerivativeAffineObservation_answer
...
theorem complexPolynomialAffineObservation_answer {d : ℕ} (p : Polynomial ℂ)
  ...
  apply complexDerivativeAffineObservation_answer
  · simpa only [Polynomial.coe_aeval_eq_eval] using p.contDiff_aeval (𝕜 := ℂ) ∞
  · intro i
    simpa only [Polynomial.coe_aeval_eq_eval] using (basis i).contDiff_aeval (𝕜 := ℂ) ∞
...
theorem ComplexAlgorithm.complexFiniteAffineFamily_run {d n : ℕ} (A : ComplexAlgorithm n)
    ...
    (A.toAffine (complexDerivativeAffineObservation f basis)).run u x =
      A.run (complexFiniteAffineFamily f basis u) x :=
  A.toAffine_run_eq _ (fun q => complexDerivativeAffineObservation_answer hf hbasis q u) x
...
theorem ComplexAlgorithm.complexPolynomialFamily_run {d n : ℕ} (A : ComplexAlgorithm n)
    ...
    (A.toAffine (complexDerivativeAffineObservation (fun z => p.eval z)
      ...
      A.run (complexFiniteAffineFamily (fun z => p.eval z)
        (fun i z => (basis i).eval z) u) x :=
  A.toAffine_run_eq _ (fun q => complexPolynomialAffineObservation_answer p basis q u) x
