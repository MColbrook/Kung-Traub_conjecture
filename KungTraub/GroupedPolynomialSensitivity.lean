...
theorem grouped_real_kernel_projected_sensitivity_lower_bound {k j : ℕ}
  ...
  exact real_kernel_projected_sensitivity_lower_bound_exponents
    (groupedStageExponent_pos sizes hsizes hjk) (groupedScalarExponent sizes)
    (fun indices hinj => sum_groupedScalarExponent_selected_le sizes hj hjk indices hinj)
    (groupedPrefixCount_le_total sizes hjk) L α x γ hγ hε hε1 hR hwmin hw ha hhalf query havoid
