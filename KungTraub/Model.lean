...
/-!
# Shared observation models and the attaining method

This module defines the real, grouped and complex oracle models, local-order
properties, counterexample witnesses, bounded stopping trees and the concrete
inverse Hermite algorithm used by the development. It imports only Mathlib.

The standalone `PalomarChallenge` copies this complete definition block in the
same order and import context. Keeping the ordinary definitions together makes
Lean's generated match and proof helpers agree across the independently
compiled reference and the proved development. The legacy definition modules
reexport these declarations under their existing import paths.

The oracle rules are total, deterministic, stationary and memoryless. Starts
are independent inputs and need not form an iteration orbit. Stopping is
represented by idle padding. A partially defined procedure transfers its local
order property through a total extension on a full punctured neighbourhood of
defined executions; the counterexample assertions concern that extension.
-/
